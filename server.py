...
class C:
    ...
    ERR = "\033[1;31m"; DIM = "\033[2m"; BOLD = "\033[1m"; RESET = "\033[0m"

def use_color():
    return sys.stdout.isatty()

def cprint(color, text):
    print((color + text + C.RESET) if use_color() else text)
...
#  Luồng bypass
# ------------------------------------------------------------------ #
def bypass(gate_url, gate_wait=15, unlock_wait=6, debug=False):
    http = Http()
    cur = gate_url
    html = None
    actions = 0
    ...
    while actions < MAX_ACTIONS:
        ...
        if action and fields:
            ...
            cur = fu
            if debug:
                cprint(C.DIM, f"   POST redirect -> {code} | {title(html)}")
            if not parse_gate_form(html)[2] and not get_unlock_token(html) and not get_key(html) and not parse_post_redirect(html)[0]:
                cprint(C.DIM, "   redirect trả trang thường -> retry sau 3s ...")
                time.sleep(3)
                code, fu, html = http.request("POST", action, fields, referer=cur)
                cur = fu
                if debug:
                    cprint(C.DIM, f"   POST redirect(retry) -> {code} | {title(html)}")
            continue
        ...
        if gfields is not None:
            ...
            if debug:
                cprint(C.DIM, f"   gate form {gmethod.upper()} {target[:56]} fields={list(gfields.keys())}")
            if gate_wait > 0 and "cad=" not in cur:
                cprint(C.DIM, f"   chờ countdown {gate_wait}s ...")
            ...
            cur = fu
            if debug:
                cprint(C.DIM, f"   submit -> {code} | {title(html)}")
            continue
        ...
        if get_unlock_token(html):
            ...
            k2 = get_k(html)
            if debug:
                cprint(C.DIM, f"   POST unlock -> {code} | newk={k2[:10] if k2 else None}")
            if k2:
                cprint(C.DIM, f"   chờ mở khóa {unlock_wait}s ...")
                time.sleep(unlock_wait)
                u3 = fu + ("&" if "?" in fu else "?") + "fc_go=1&k=" + urllib.parse.quote(k2)
                code, fu, html = http.request("GET", u3, referer=fu)
                cur = fu
                if debug:
                    cprint(C.DIM, f"   GET fc_go -> {code} | {title(html)}")
            continue
    ...
    def do_POST(self):
        try:
            ...
            if not target_url:
                ...
                self.wfile.write(json.dumps({"status": "error", "message": "Thiếu tham số url"}).encode('utf-8'))
                return

            cprint(C.HEAD, f"\n[API] Nhận yêu cầu bypass link: {target_url}")
            
            # Chạy hàm bypass cốt lõi
            key = bypass(target_url, gate_wait=10, unlock_wait=4, debug=True)
            ...
            if key:
                ...
                self._set_headers(400)
                self.wfile.write(json.dumps({"status": "error", "message": "Không bóc được key, link có thể đã hết hạn."}).encode('utf-8'))

        except Exception as e:
            cprint(C.ERR, f"[x] Lỗi Server: {e}")
            self._set_headers(500)
            self.wfile.write(json.dumps({"status": "error", "message": str(e)}).encode('utf-8'))

    def log_message(self, format, *args):
        # Tắt log mặc định rườm rà của BaseHTTPRequestHandler, dùng cprint tự quản lý
        return

def run(port=8080):
    server_address = ('', port)
    httpd = HTTPServer(server_address, BypassServer)
    cprint(C.OK, f"==================================================")
    cprint(C.BOLD, f"  BYPASS SERVER ĐANG CHẠY TẠI CỔNG {port}")
    cprint(C.OK, f"==================================================")
    try:
        httpd.serve_forever()
